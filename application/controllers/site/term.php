...
class Term extends CI_Controller {
	...
	public function index($type, $alias, $page=1){
		if(empty($page))$page=1;
		$this->load->model("config_model");
		$this->load->model("term_model");
x		
		$this->term_model->getTermByAlias($alias);
		if($this->term_model->id){
			...
			$this->db->join('terms', 'terms.id = term_node.term_id','inner');
			$this->db->where('terms.alias', $alias); 
			...
			if(!empty($nodes)){
				...
				$config["root_url"]='/term/'.$this->term_model->type.'/'.$this->term_model->alias.'/page/';
